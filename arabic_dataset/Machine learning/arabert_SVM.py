"""AraBERT + PCA + Juliette's SVM on Jean's final Arabic modeling dataset.

This script keeps Juliette's SVMClassifier class exactly as provided and uses it
on the same final acceptable AraBERT pathway previously selected for Jean's data:
...
└── arabert_pca_svm_juliette_on_jean_dataset.py
"""
...
@dataclass
class AraBERTPCASVMConfig:
    dataset_path: str = "final_modeling_dataset.csv"
    output_dir: str = "arabert_pca_svm_juliette_outputs"
...
class ModelingDatasetLoader:
    ...
    def __init__(self, config: AraBERTPCASVMConfig) -> None:
        self.config = config
...
def run_experiment() -> None:
    ...
    results_df, y_pred = svm.evaluate(
        ...
        model_name="AraBERT Mean + PCA96 + Juliette SVM",
    )
    ...
    summary_lines = [
        "AraBERT + PCA + Juliette SVM Summary",
        "=====================================",
        "",
        "Vectorizer:",
