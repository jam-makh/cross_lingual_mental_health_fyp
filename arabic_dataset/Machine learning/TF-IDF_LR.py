"""Light TF-IDF + Logistic Regression on Jean's final Arabic modeling dataset.

This script uses the team LogisticRegressionModel class and applies it
to a lighter TF-IDF setup, following the same lighter reduction previously used
for the SVM experiment to keep training practical.

The dataset treatment remains unchanged:
...
- Lighter TF-IDF representation:
    * analysis_text word TF-IDF only
    * n-grams (1, 2), min_df=3, max_features=30,000
- Classifier:
...
└── light_tfidf_logistic_regression_on_jean_dataset.py
"""
...
class TFIDFLogisticConfig:
    ...
    output_dir: str = "light_tfidf_logistic_regression_outputs"
...
class ModelingDatasetLoader:
    ...
    def __init__(self, config: TFIDFLogisticConfig) -> None:
        self.config = config
...
def run_experiment() -> None:
    ...
    # The prior 100,000-feature hybrid representation was intentionally reduced
    # to match the lighter TF-IDF setup already used for the SVM experiment.
    ...
    results_df, y_pred = evaluate_predictions(
        ...
        model_name="Light TF-IDF + Logistic Regression",
    )
    ...
    summary_lines = [
        "Light TF-IDF + Logistic Regression Summary",
        "======================================",
        "",
        "Vectorizer:",
        "- Lighter word-only TF-IDF path, matching the lighter SVM experiment",
        f"- Word TF-IDF: analysis_text, n-grams={config.word_ngram_range}, min_df={config.word_min_df}, max_features={config.word_max_features}",
        f"- Retained features in lighter TF-IDF matrix: {X_train.shape[1]:,}",
        "",
        "Data:",
    ...
    plot_confusion_matrix_for_model(
        ...
        y_pred,
        label_encoder=label_encoder,
        title="Confusion Matrix — Light TF-IDF + Logistic Regression",
    )
